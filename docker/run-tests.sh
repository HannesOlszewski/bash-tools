...
cd "$(dirname "$0")/.."
img=${1:-arch}
docker build -q -t bash-tools-test-$img -f docker/Dockerfile.$img docker >/dev/null
docker run --rm -t -v "$PWD":/src:ro -e CARGO_TARGET_DIR=/tmp/target bash-tools-test-$img bash -c '
  set -e
  cp -r /src /work/repo && cd /work/repo
  bash --version | head -1
  cargo test --release -q 2>&1 | tail -5
